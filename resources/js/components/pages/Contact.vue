<template>
  <main class="main">
    <div class="contact-form-container login-form-container animate-in">
      ...
      <form id="contact-feedback-form" @submit.prevent="submitForm">
        <div class="form-group reveal-on-scroll" data-reveal-delay="150">
          <label for="contact-name">Emri Juaj (opsionale)</label>
          <div class="input-icon-wrapper">
            <i class="fas fa-user input-icon"></i>
            <input type="text" id="contact-name" name="contact_name" placeholder="Emri juaj" v-model="form.name">
          </div>
        </div>
        <div class="form-group reveal-on-scroll" data-reveal-delay="200">
          ...
          <div class="input-icon-wrapper">
            ...
            <input type="email" id="contact-email" name="contact_email" placeholder="email@shembull.com" v-model="form.email">
          </div>
        </div>
        <div class="form-group reveal-on-scroll" data-reveal-delay="250">
          ...
          <div class="input-icon-wrapper">
            ...
            <input type="text" id="feedback-subject" name="feedback_subject" required placeholder="P.sh., Sugjerim për model të ri, Problem teknik" v-model="form.subject">
          </div>
        </div>
        <div class="form-group full-width reveal-on-scroll" data-reveal-delay="300">
          ...
          <textarea id="feedback-message" name="feedback_message" rows="6" required placeholder="Shkruani detajet këtu..." v-model="form.message"></textarea>
      </div>
        <button type="submit" class="btn-primary-form btn btn-primary reveal-on-scroll" data-reveal-delay="350">
          Dërgo Mesazhin <i class="fas fa-paper-plane icon-right"></i>
        </button>
      </form>
...
export default {
  ...
  setup() {
    ...
    const submitForm = () => {
      ...
      messageArea.className = 'form-message message success';
      messageArea.textContent = 'Faleminderit për mesazhin tuaj! (Ky është një demonstrim)';
...
.login-form-container { /* Reusing login form container style */
  max-width: 800px;
}
</style>