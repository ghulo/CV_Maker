<template>
  <div class="modern-mini-cv-content p-4 text-neutral-text dark:text-dark-neutral-text border border-neutral-border dark:border-dark-neutral-border rounded-lg scale-75 origin-top-left overflow-hidden h-64 w-full relative flex">
    <!-- Left Column: Personal Info (scaled) -->
    <div class="modern-mini-sidebar w-1/3 pr-4 border-r border-divider-color dark:border-dark-divider-color">
      <h3 class="text-base font-bold text-primary dark:text-primary-accent mb-2">{{ cv.personal_details.full_name || 'Emri Mbiemri' }}</h3>
      <p class="text-xs text-muted-text dark:text-dark-muted-text">{{ cv.personal_details.email || 'email@example.com' }}</p>
      <p class="text-xs text-muted-text dark:text-dark-muted-text mb-4">{{ cv.personal_details.phone_number || '555-1234' }}</p>
      ...
      <section class="mt-4">
        ...
        <div class="flex flex-wrap gap-1">
          <span v-for="(skill, index) in (cv.skills || []).slice(0, 3)" :key="index"
    ...
    <div class="modern-mini-main-content w-2/3 pl-4">
      <section class="mb-4">
        <h4 class="text-sm font-semibold text-neutral-text dark:text-dark-neutral-text border-b border-primary dark:border-dark-primary pb-1 mb-2">Përmbledhja</h4>
        <p class="text-xs text-subtle-text dark:text-dark-subtle-text line-clamp-3">{{ cv.summary || 'Përshkrim i shkurtër i karrierës...' }}</p>
      </section>
      ...
      <section class="mb-4">
        ...
        <p v-else class="text-xs text-muted-text dark:text-dark-muted-text">Nuk ka përvojë të shtuar.</p>
      </section>
      ...
      <section>
        ...
        <p v-else class="text-xs text-muted-text dark:text-dark-muted-text">Nuk ka edukim të shtuar.</p>
      </section>
    </div>
