<template>
  <div
    class="professional-mini-cv-content p-4 text-neutral-text dark:text-dark-neutral-text border border-neutral-border dark:border-dark-neutral-border rounded-lg scale-75 origin-top-left overflow-hidden h-64 w-full relative"
  >
    <!-- Header: Name and Summary (scaled) -->
    <header
      class="text-center mb-4 pb-2 border-b border-divider-color dark:border-dark-divider-color"
    >
      <h3 class="text-lg font-bold text-primary dark:text-primary-accent mb-1">
        {{ cv.personalDetails.fullName || 'Emri Mbiemri' }}
      </h3>
      <p class="text-xs text-muted-text dark:text-dark-muted-text line-clamp-1">
        ...
        <section class="mb-4">
          ...
          <ul class="list-disc list-inside space-y-0.5 text-xs">
            <li
              ...
              :key="index"
              class="text-subtle-text dark:text-dark-subtle-text"
            >
              {{ skill.name }}
            </li>
      ...
      <div class="col-span-2 pl-2">
        <section>
          ...
          <div v-if="cv.experience && cv.experience.length">
            ...
            <p class="text-xs text-subtle-text dark:text-dark-subtle-text line-clamp-2">
              {{ cv.experience[0].description || 'Përshkrim i shkurtër detyrash.' }}
            </p>
          </div>
